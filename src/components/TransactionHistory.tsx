...
const TransactionHistory: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (status) {
      case 'completed':
        return <span className={`${baseClasses} bg-green-100 text-green-600`}>Thành công</span>;
      case 'pending':
        return <span className={`${baseClasses} bg-amber-100 text-amber-600`}>Chờ xử lý</span>;
      case 'failed':
        return <span className={`${baseClasses} bg-red-100 text-red-600`}>Thất bại</span>;
      default:
  ...
  return (
    <div className="transaction-history">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-xl font-extrabold text-gray-800">Lịch Sử Giao Dịch</h3>
      </div>
      ...
      {transactions.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          ...
          <p className="text-xs text-gray-300 mt-1 uppercase tracking-widest font-bold">Bắt đầu nạp token để trải nghiệm ngay</p>
        </div>
      ) : (
          ...
          {transactions.map((tx) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="font-bold text-gray-800 text-sm">{tx.description || 'Giao dịch token'}</p>
              ...
              <div className="text-right">
                <p className={`text-lg font-black ${tx.amount > 0 ? 'text-green-600' : 'text-blue-600'}`}>
                  {tx.amount > 0 ? `+${tx.amount.toLocaleString()}` : tx.amount.toLocaleString()}
                </p>
                <p className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Tokens</p>
              </div>
            </div>
          ))}
        </div>
